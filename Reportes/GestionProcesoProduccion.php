    ...
    // --------------------METODO PARA ADAPTAR LAS CELDAS------------------------------
        ...
        function CheckPageBreak($h, $setX) {
            ...
            if ($this->GetY() + $h > $this->PageBreakTrigger) {
                ...
                $this->Cell(70,8,utf8_decode('ESTADO DEL PROCESO'),1,0,'C',0);  //C=Center, R=right, L=left
                $this->Cell(100,8,utf8_decode('FECHA'),1,1,'C',0);  //C=Center, R=right, L=left
...
//------------------OBTENES LOS DATOS DE LA BASE DE DATOS-------------------------
 ...
 $sql = "SELECT * FROM tbl_proceso_produccion";
...
$pdf->SetAutoPageBreak(true, 20); //salto de pagina automatico
    ...
    $pdf->Cell(100,55,utf8_decode('Reporte de Procesos de Producción'),0,0,'C',0);//Nombre del reporte 
    $pdf->SetFont('Helvetica','B',15); //Definir tipo de letra y tamaño B=Negrita
    $pdf->SetTextColor(0,0,0); //Color negro
    $pdf->Ln(40);
    $pdf->Cell(15,8,utf8_decode('N°'),1,0,'C',0);  //C=Center, R=right, L=left
    $pdf->Cell(70,8,utf8_decode('ESTADO DEL PROCESO'),1,0,'C',0);  //C=Center, R=right, L=left
    $pdf->Cell(100,8,utf8_decode('FECHA'),1,1,'C',0);  //C=Center, R=right, L=left
    // -------TERMINA----ENCABEZADO------------------
...
//$pdf->SetAligns(array('C','C','C','L'));
 ...
 while ($fila = mysqli_fetch_assoc($resultado)) {
    $pdf->Cell(15,8, ucwords(strtolower(utf8_decode($fila['Id_Proceso_Produccion']))),1,0,'C',0);
    $pdf->Cell(70,8, ucwords(strtolower(utf8_decode($fila['Id_Estado_Proceso']))),1,0,'C',0);
    $pdf->Cell(100,8, ucwords(strtolower(utf8_decode($fila['Fecha']))),1,1,'C',0);
 }
